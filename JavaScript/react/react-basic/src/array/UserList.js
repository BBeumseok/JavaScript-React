...
function User({user, onRemove, onToggle}) {
  ...
  언마운트 시에 하는 작업들
  1. setInterval, setTimeout을 사용하여 등록한 작업들 clear 하기
    (clearInterval, clearTimeout)
  2. 라이브러리 인스턴스 제거
  */
  // useEffect(() => {
  //   console.log('컴포넌트가 화면에 나타남');
  //   return () => {   //  함수 종료 시... => cleanup
  ...
  /* 
  의존성 배열에 특정 값 넣기
  -> 컴포넌트가 처음 마운트 될때 호출되고, 지정한 값(의존성)이 바뀔 때마다
    호출됩니다.
  ...
  */
  ...
  /* 
    ...
    왜? 의존성 배열에 넣지 않으면, useEffect()에 등록된 함수가 실행될 때
    최신 props 또는 상태를 가리키지 못하게 됩니다.
   */ 
  /* 
    의존성 배열을 생략하는 경우 인자값을 삭제하는 경우는 리랜더링 시마다
    호출이 됩니다.
  */
